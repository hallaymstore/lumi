...
const networkFormats = {
  native: { type: 'native', src: process.env.AD_NETWORK_NATIVE || 'https://pl29295128.profitablecpmratenetwork.com/e884bc1779f790089be3868bcb7ae57b/invoke.js', container: 'container-e884bc1779f790089be3868bcb7ae57b', width: 300, height: 250 },
...
router.get('/ads/network/direct', (req, res) => {
  ...
  try { const candidate = new URL(process.env.AD_NETWORK_DIRECT_URL || target); if (candidate.protocol === 'https:') target = candidate.href; } catch {}
  const placement = cleanPlacement(req.query.placement);
  const format = ['native', 'mobile', 'rectangle', 'desktop', 'leaderboard', 'direct'].includes(String(req.query.format)) ? String(req.query.format) : 'direct';
  recordNetworkAdEvent(req, res, { type: 'click', placement, format, providerState: 'direct' })
    .catch(() => {}).finally(() => res.set('Cache-Control', 'no-store').redirect(302, target));
});

router.get('/ads/network/:format', (req, res) => {
  const format = networkFormats[req.params.format];
  if (!format) return res.sendStatus(404);
  const providerUrl = format.type === 'native' ? format.src : `https://www.highperformanceformat.com/${format.key}/invoke.js`;
  const container = format.type === 'native' ? `<div id="${format.container}" class="native-slot"></div>` : '';
  const setup = format.type === 'native' ? '' : `window.atOptions={key:${JSON.stringify(format.key)},format:'iframe',height:${format.height},width:${format.width},params:{}};`;
  res.set({
    'Cache-Control': 'public, max-age=120',
    'Content-Security-Policy': "default-src 'none'; script-src 'unsafe-inline' https://www.highperformanceformat.com https://*.profitablecpmratenetwork.com; frame-src https:; connect-src https:; img-src https: data:; style-src 'unsafe-inline'",
    'Referrer-Policy': 'no-referrer-when-downgrade'
  });
  res.type('html').send(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:transparent}body{display:grid;place-items:center}.native-slot{width:100%;height:100%;overflow:hidden}</style></head><body>${container}<script>(()=>{let settled=false;const notify=state=>{if(settled)return;settled=state==='ready';parent.postMessage({type:'lumi-adsterra',state},location.origin)};${setup}const script=document.createElement('script');script.async=true;script.src=${JSON.stringify(providerUrl)};script.onerror=()=>notify('error');document.body.appendChild(script);const hasCreative=()=>${format.type === 'native' ? `document.getElementById(${JSON.stringify(format.container)})?.childElementCount>0` : `!!document.querySelector('iframe')`};const watch=setInterval(()=>{if(hasCreative()){clearInterval(watch);notify('ready')}},250);setTimeout(()=>{clearInterval(watch);if(!settled)notify('empty')},6500)})();<\/script></body></html>`);
});

router.post('/api/ads/network/:type', async (req, res) => {
  if (!['impression'].includes(req.params.type)) return res.status(400).json({ ok: false });
  const format = ['native', 'mobile', 'rectangle', 'desktop', 'leaderboard'].includes(String(req.body.format)) ? String(req.body.format) : 'native';
  const providerState = req.body.providerState === 'provider' ? 'provider' : 'fallback';
  const result = await recordNetworkAdEvent(req, res, { type: req.params.type, placement: cleanPlacement(req.body.placement), format, providerState });
